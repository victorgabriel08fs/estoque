        ...
        <div class="card mb-5">
            ...
            <form action="{{ route('products.index') }}">
                ...
                <div class="card-footer d-flex justify-content-between">
                    <div>
                        <a href="{{ route('products.index') }}" class="btn btn-outline-secondary">Clear fields</a>
                        <button type="submit" class="btn btn-outline-primary">Search</button>
                    </div>
            </form>
            <div>
                @can('Create Products')
                    @include('products._partials.new_modal')
                @endcan
            </div>
        </div>
    </div>
                    ...
                    <tbody>
                        @foreach ($products as $item)
                            <tr>
                                ...
                                <td>{{ $item['name'] }}</td>
                                <td title="{{ $item['describe'] }}">{{ $item['name'] }}</td>
                                <td>{{ $item->category->name }}</td>
                                <td>{{ $item->inStorages() }}</td>
                                <td><span
                                        class="status status-{{ $item->active ? 'green' : 'red' }}">{{ $item->active ? 'Active' : 'Desactive' }}</span>
                                </td>
                                <td>
                                    <div class="btn-group">
                                        <a class="btn btn-icon btn-primary" href="{{ route('products.show', $item) }}"><svg
                                        ...
                                        @can('Edit Products')
                                            @include('products._partials.edit_modal', [
                                                'product' => $item,
                                            ])
                                        @endcan
                                        @can('Delete Products')
